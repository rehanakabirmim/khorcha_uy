...
@section('main-content')
  <div class="row">
      <div class="col-md-12 ">
          <form method="post" action="{{url('dashboard/expense/insert')}}">
              @csrf
              <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3 {{ $errors->has('category') ? ' has-error' : '' }}">
                      ...
                      <div class="col-sm-7">
                        @php
                          $allCate=App\Models\ExpenseCategory::where('expcat_status',1)->orderBy('expcat_name','ASC')->get();
                        @endphp
                        <select class="form-control form_control" id="" name="category">
                          <option value="">Choose Category</option>
                          @foreach($allCate as $cate)
                          <option value="{{$cate->expcat_id}}">{{$cate->expcat_name}}</option>
                          @endforeach
                        </select>
